...
function preload() {
  ...
  smLogo = loadImage('assets/android-icon-192x192.png');
  ...
  smgSans = loadFont("assets/SMGSans-Regular.woff")
}
...
function setup() {
  ...
  butPos1 = [cnv.width/2 - timeSpec.width/2, cnv.height * 0.7];
  ...
  randomObj.position(butPos2[0], butPos2[1] + cnv.width * 0.031);
  randomObj.style('background-color', colT);
  randomObj.style('border', colT);
  randomObj.style('color', colT2);
  randomObj.style('font-size', cnv.width*0.008 + 'px');
...
function initiate() {  
  ...
  textFont(smgSans);
  imageMode(CENTER);
  image(map, cnv.width/2, cnv.height/3, 0.488*cnv.width, (0.488*cnv.width)*0.611);
  imageMode(CORNER);
...
function drawBox() {
  ...
  if (bWidth > cnv.width * 0.26) {
    ...
    let tX = boxX - boxWidth/2 + activeImg.width/2 + cnv.width * 0.031;
    text(eval(activeObj).title, tX, cnv.height/9, cnv.width * 0.115, cnv.width * 0.156);
    textWrap(WORD);
    text(eval(activeObj).des, tX, cnv.height/9 + cnv.width*0.016, cnv.width*0.115, cnv.width*0.156);
  }
}

function drawConnections() {
  strokeWeight(cnv.width * 0.003);
