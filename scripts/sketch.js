...
function drawBox() {
  ...
  if (bWidth > cnv.width * 0.26) {
    image(mediaBar, cnv.width*0.285, cnv.height*0.55, mediaBar.width * cnv.width*0.000521, mediaBar.height * cnv.width*0.00055);
...
function galleryBar () {
  if (galleryBarState) {
      imageMode(CORNER);
      image(galleryIcon, 0, cnv.height * 0.131, galleryIcon.width * cnv.width*0.000505, galleryIcon.height * cnv.width*0.000515);
  }
  else if (!galleryBarState) {
      imageMode(CORNER);
      image(galleryBarClosed, 0, cnv.height * 0.131, galleryBarClosed.width * cnv.width*0.0005, galleryBarClosed.height * cnv.width*0.000515);
  }
}
...
function gamesBar() {
  if (gameBarState) {
    imageMode(CORNER);
    image(gameBar, 0, cnv.height * 0.38, gameBar.width * cnv.width * 0.000525, gameBar.height * cnv.width*0.000518);
  }
  else if (!gameBarState) {
    imageMode(CORNER);
    image(gameBarClosed, 0, cnv.height * 0.38, gameBarClosed.width * cnv.width*0.0005, gameBarClosed.height * cnv.width*0.000518);
    gameOn.style('display', 'none');
  }
}
